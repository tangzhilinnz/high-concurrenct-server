...
MemPool::Malloc(size_t size)
{
	...
	if ((tc->_freeList[index])._currentHead == nullptr)
	{
		if ((tc->_freeList[index])._depsoitHead != nullptr)
		{
			(tc->_freeList[index])._currentHead = (tc->_freeList[index])._depsoitHead;
			(tc->_freeList[index])._currentTail = (tc->_freeList[index])._depsoitTail;
			(tc->_freeList[index])._depsoitHead = nullptr;
			(tc->_freeList[index])._depsoitTail = nullptr;
			(tc->_freeList[index])._currentNum = __BATCH_SIZE;
		}
		else //(tc->_freeList[index])._depsoitHead == nullptr
		{
			...
			if (__freeList[index]._batchNum != 0)
			{
				(tc->_freeList[index])._currentHead = __freeList[index]._batchHead;
				(tc->_freeList[index])._currentTail = __freeList[index]._batchTail;
				(tc->_freeList[index])._currentNum += __BATCH_SIZE;
				//(tc->_freeList[index])._classNum += __BATCH_SIZE;
				__freeList[index]._batchHead = 
					*reinterpret_cast<u_char**>(__freeList[index]._batchHead + 8);
				__freeList[index]._batchTail =
					*reinterpret_cast<u_char**>(__freeList[index]._batchTail + 8);
				--__freeList[index]._batchNum;
	...
	u_char* m = (tc->_freeList[index])._currentHead;
	(tc->_freeList[index])._currentHead = 
		*reinterpret_cast<u_char**>((tc->_freeList[index])._currentHead);
	if ((tc->_freeList[index])._currentHead == nullptr)
		(tc->_freeList[index])._currentTail = nullptr;
	--(tc->_freeList[index])._currentNum;
...
MemPool::Free(void* p)
{
	...
	if ((tc->_freeList[index])._currentNum < __BATCH_SIZE)
	{
		*reinterpret_cast<u_char**>(p) = (tc->_freeList[index])._currentHead;
		(tc->_freeList[index])._currentHead = (u_char*)p;
		++(tc->_freeList[index])._currentNum;

		if ((tc->_freeList[index])._currentTail == nullptr)
			(tc->_freeList[index])._currentTail = (u_char*)p;
	}
	else //(tc->_freeList[index])._currentNum == __BATCH_SIZE
	{
		if ((tc->_freeList[index])._depsoitHead == nullptr)
		{
			(tc->_freeList[index])._depsoitHead = 
				(tc->_freeList[index])._currentHead;
			(tc->_freeList[index])._depsoitTail =
				(tc->_freeList[index])._currentTail;

			*reinterpret_cast<u_char**>(p) = nullptr;
			(tc->_freeList[index])._currentHead = (u_char*)p;
			(tc->_freeList[index])._currentTail = (u_char*)p;
			(tc->_freeList[index])._currentNum = 1;
			//++(tc->_freeList[index])._classNum;
		}
		else //(tc->_freeList[index])._depsoitHead != nullptr
		{
			...
			{usleep(0);}*/
			{
				...
				*reinterpret_cast<u_char**>((tc->_freeList[index])._currentHead + 8) =
					__freeList[index]._batchHead;
				__freeList[index]._batchHead = (tc->_freeList[index])._currentHead;

				*reinterpret_cast<u_char**>((tc->_freeList[index])._currentTail + 8) =
					__freeList[index]._batchTail;
				__freeList[index]._batchTail = (tc->_freeList[index])._currentTail;
