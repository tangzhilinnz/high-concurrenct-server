...
MemPool::SetClass(int classSize, int unitNum)
{
	...
	for (int i = 0; i < 256 * unitNum; i++)
	{
		...
		{
			//第一个链表元素的next指针内容nullptr
			*reinterpret_cast<u_char**>(pCutting) = pBatch;
			pBatch = pCutting;
			pCutting += classSize;
		}
		*reinterpret_cast<u_char**>(pBatch + sizeof(u_char*)) = pBatchTail;
	...
	if (pClass->_currentHead == nullptr)
	{
		...
		else //(tc->_freeList[index])._depsoitHead == nullptr
		{
			...
			if (pUnit->_batchNum != 0)
			{
				...
				pUnit->_batchHead = 
					*reinterpret_cast<u_char**>(pClass->_currentTail);
				*reinterpret_cast<u_char**>(pClass->_currentTail) = nullptr; 
				pClass->_currentNum += __BATCH_SIZE;
				--pUnit->_batchNum;
