...
#这里是诸多.d文件被包含进来，每个.d文件里都记录着一个.o文件所依赖哪些.c和.h文件。内容诸如 nginx.o: nginx.cpp ngx_func.h
...
#那一个.o依赖于哪些.h文件，我们可以用“g++ -MM c程序文件名” 来获得这些依赖信息并重定向保存到.d文件中
...
#g++ -MM $^ | sed 's,\(.*\)\.o[ :]*,$(LINK_OBJ_DIR)/\1.o:,g' > $@
#echo 中 -n表示后续追加不换行
	echo -n $(LINK_OBJ_DIR)/ > $@
#	g++ -MM $^ | sed 's/^/$(LINK_OBJ_DIR)&/g' > $@
#  >>表示追加
#	g++ -I$(INCLUDE_PATH) -MM $^ >> $@
